"""
Vertex view CubeBench environment for 3D perspective learning.
"""
...
class VertexViewEnv(CubeBenchEnv):
    """
    Vertex view environment for 3D perspective learning.
    
    State: (cube_state_string, view_id)
    Observation: 256x256 RGB image from current vertex view
    Actions: 12 cube moves + 3 view transitions
    """
    ...
    def __init__(self,
        ...
        """
        Initialize vertex view environment.
        ...
        """
        # Initialize vertex-specific attributes first
        self.vertex_renderer = VertexViewRenderer(
            image_size=256,
            show_face_labels=show_face_labels
        )
        self.current_view_id = 0  # Start at view 0
        self.view_neighbors = get_view_neighbors(ViewType.VERTEX)
        ...
        action_space_config = ActionSpace(
            ...
            custom_view_actions=['view_axis1', 'view_axis2', 'view_axis3']
        )
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[Dict[str, Any]] = None) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        observation = self.vertex_renderer.get_observation(self.current_state, self.current_view_id)
        
        # Update info
        info['state_string'] = self.current_state_str
        info['view_id'] = self.current_view_id
        info['view_type'] = 'vertex'
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        ...
        Args:
            action: Action index (0-11 for cube moves, 12-14 for view transitions)
    ...
    def get_next_state_observation(self, state_str: str, view_id: int, action: int) -> Tuple[Tuple[str, int], np.ndarray]:
        ...
        observation = self.vertex_renderer.get_observation(next_state_array, next_view_id)
    ...
    def _apply_view_transition(self, action_name: str) -> bool:
        ...
        Args:
            action_name: View action name ('view_axis1', 'view_axis2', 'view_axis3')
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        return self.vertex_renderer.get_observation(self.current_state, self.current_view_id)
    ...
    def cleanup(self):
        """Clean up resources"""
        if hasattr(self.vertex_renderer, 'cleanup'):
            self.vertex_renderer.cleanup() 